...
| **refresh token** | 256 位随机串、**库中存哈希、DB 托管**、24h 滑动、旋转式 | **仅**用于 `POST /api/client/auth/renew` | 因其不透明且落库，服务端可**即时作废/轮换**——这是单签名令牌做不到的：支撑 AC6 静默续期（无需重输密钥）、AC3 旧凭据立即失效（轮换删旧插新）、AC8 禁用即让 refresh 记录失效迫使重激活、AC12 在途请求不失败（旧 access jti 进 60s 宽限名单） |
...
| 幂等 / 防重 | 激活幂等（同 keyId+fingerprint 重复激活返回已激活）；翻译幂等（§9.2） |
